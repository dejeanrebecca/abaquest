...
export function QuestWelcome({
  ...
  const emotions = [
    { emoji: '😊', label: 'Excited!', value: 'excited', color: 'bg-green-100', icon: Laugh },
    { emoji: '🙂', label: 'Good', value: 'good', color: 'bg-blue-100', icon: Smile },
    { emoji: '😐', label: 'Okay', value: 'okay', color: 'bg-yellow-100', icon: Meh },
    { emoji: '😟', label: 'Nervous', value: 'nervous', color: 'bg-orange-100', icon: Frown },
  ];

  const [selectedEmotion, setSelectedEmotion] = React.useState('');

  const handleNext = () => {
    if (showEmotionalCheckIn && selectedEmotion) {
      onEmotionalCheckIn?.(selectedEmotion);
    }
    onNext();
  };
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-6">
          <div className="text-6xl mb-4">{questIcon}</div>
        ...
        {showEmotionalCheckIn && (
          <div className="mb-8">
            <p className="text-center text-deep-blue/80 text-xl mb-6">
        ...
        >
          <Button
            onClick={handleNext}
            disabled={showEmotionalCheckIn && !selectedEmotion}
